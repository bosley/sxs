...
void slp_process_group(slp_scanner_t *scanner, uint8_t start, uint8_t end,
  ...
  if (content_len > 0) {
    ...
    if (sub_buffer && bytes_copied > 0) {
      ...
      if (sub_scanner) {
        slp_process_tokens(sub_scanner, state, stops, depth + 1);
...
void slp_process_tokens(slp_scanner_t *scanner, slp_processor_state_t *state,
  ...
  while (scanner->position < scanner->buffer->count) {
    ...
    slp_print_static_type(&result.data, depth);
    state->tokens_processed++;
  }
}
...
int slp_process_buffer(slp_buffer_t *buffer) {
  ...
  slp_process_tokens(scanner, &state, &stops, 0);
...
int slp_process_file(char *file_name) {
  ...
  int return_value = slp_process_buffer(buffer);
