...
/*
    ...
    One map is for compiling the the command to our target that will use the
   emitter in-context to emit instructions after validating all type data with
   the type system

    This way we can build out the core of the language and it can
   compile/interpret. this way when we build our final prouct, it can use itself
   as a library and interpret at runtime as well.
*/
