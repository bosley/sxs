...
get_standard_callable_symbols() {
  ...
  symbols["set"] = callable_symbol_s{
      ...
      .function = [](callable_context_if &context,
        ...
        if (symbol_obj.type() != slp::slp_type_e::SYMBOL) {
          throw std::runtime_error("set requires first argument to be a symbol");
        }
  ...
  symbols["fn"] = callable_symbol_s{
      ...
      .function = [](callable_context_if &context,
        ...
        auto list = args_list.as_list();
        if (list.size() != 4) {
          throw std::runtime_error("fn requires exactly 3 arguments: (params) :return-type [body]");
        }
        ...
        if (body_obj.type() != slp::slp_type_e::BRACKET_LIST) {
          throw std::runtime_error("fn: third argument must be bracket list (function body)");
        }
        ...
        if (!context.is_symbol_enscribing_valid_type(return_type_sym, return_type)) {
          throw std::runtime_error(fmt::format("fn: invalid return type: {}", return_type_sym));
        }
        ...
        for (size_t i = 0; i < params_list.size(); i += 2) {
          ...
          if (!context.is_symbol_enscribing_valid_type(param_type_sym, param_type)) {
            throw std::runtime_error(fmt::format("fn: invalid parameter type: {}", param_type_sym));
          }
