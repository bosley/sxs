...
event_system_c::event_system_c(runtime::logger_t logger, size_t max_threads, size_t max_queue_size)
  : logger_(logger), 
    running_(false), 
    max_threads_(max_threads), 
    max_queue_size_(max_queue_size),
    shutdown_requested_(false) {
  logger_->info("[{}] Created with {} worker threads and queue size {}", 
    name_, max_threads_, max_queue_size_);
}
...
void event_system_c::initialize(runtime::runtime_accessor_t accessor) {
  ...
  running_ = true;
  logger_->info("[{}] Event system initialized with {} threads", name_, worker_threads_.size());
}
...
void event_system_c::shutdown() {
  ...
  logger_->info("[{}] Waiting for {} worker threads to complete", name_, worker_threads_.size());
  size_t thread_idx = 0;
  ...
  
  {
    ...
    if (remaining > 0) {
      logger_->warn("[{}] Clearing {} unprocessed events from queue", name_, remaining);
    }
  ...
  while (true) {
    event_s event;
    {
      ...
      if (shutdown_requested_ && event_queue_.empty()) {
        logger_->info("[{}] Worker thread {:x} shutting down", name_, thread_hash);
        break;
      }
    ...
    if (it != topic_consumers_.end()) {
      for (auto& consumer : it->second) {
        try {
          ...
          logger_->error("[{}] Consumer exception for topic {}: {}", 
            name_, 
            event.topic_identifier, 
            e.what());
        } catch (...) {
          logger_->error("[{}] Unknown consumer exception for topic {}", 
            name_, 
            event.topic_identifier);
        }
      }
    } else {
      logger_->debug("[{}] No consumers registered for topic {}", 
        name_, 
        event.topic_identifier);
    }
  }
