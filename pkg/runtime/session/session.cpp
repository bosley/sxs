...
std::string session_c::get_entity_id() const { return entity_id_; }

std::string session_c::get_scope() const { return scope_; }

bool session_c::is_active() const { return active_; }

std::time_t session_c::get_creation_time() const { return creation_time_; }

void session_c::set_active(bool active) { active_ = active; }
...
session_subsystem_c::session_subsystem_c(logger_t logger,
                                         size_t max_sessions_per_entity)
    : logger_(logger), max_sessions_per_entity_(max_sessions_per_entity),
      running_(false), session_store_(nullptr), datastore_(nullptr),
      entity_store_(nullptr), event_system_(nullptr), session_counter_(0) {}

const char *session_subsystem_c::get_name() const { return name_; }
...
bool session_subsystem_c::is_running() const { return running_; }

std::string
session_subsystem_c::generate_session_id(const std::string &entity_id) {
...
bool session_subsystem_c::persist_session_metadata(const session_c &session) {
  ...
  std::ostringstream oss;
  oss << session.get_entity_id() << "|" << session.get_scope() << "|"
...
std::shared_ptr<session_c>
session_subsystem_c::create_session(const std::string &entity_id,
  ...
  if (current_count >= max_sessions_per_entity_) {
    logger_->error(
        "[{}] Cannot create session for entity {}: max sessions ({}) reached",
  ...
  if (!entity) {
    logger_->error("[{}] Cannot create session: entity {} not found", name_,
                   entity_id);
    return nullptr;
  }
  ...
  logger_->info("[{}] Created session {} for entity {} with scope {}", name_,
                session_id, entity_id, scope);

  return session;
}

std::shared_ptr<session_c>
session_subsystem_c::get_session(const std::string &session_id) {
...
session_subsystem_c::list_sessions_by_entity(const std::string &entity_id) {
  std::vector<std::shared_ptr<session_c>> result;
  for (const auto &pair : sessions_) {
...
bool session_subsystem_c::close_session(const std::string &session_id) {
  auto it = sessions_.find(session_id);
  if (it == sessions_.end()) {
    logger_->error("[{}] Cannot close session {}: not found", name_,
                   session_id);
    return false;
  }
...
bool session_subsystem_c::destroy_session(const std::string &session_id) {
  auto it = sessions_.find(session_id);
  if (it == sessions_.end()) {
    logger_->error("[{}] Cannot destroy session {}: not found", name_,
                   session_id);
    return false;
  }
...
void session_subsystem_c::set_entity_store(kvds::kv_c *store) {
  ...
  if (entity_store_) {
    ...
    entity_manager_ = std::make_unique<record::record_manager_c>(*entity_store_,
                                                                 spdlog_logger);
    logger_->info("[{}] Entity manager initialized", name_);
  }
}

void session_subsystem_c::set_event_system(
    events::event_system_c *event_system) {
  event_system_ = event_system;
