...
public:
  ...
  /*
    The entity storage is our records of users/entities that can interact with the system
  */
  kvds::kv_c* get_entity_store();

  /*
    The session storage is to disk-back all entity actions that are happening via a session
    object. These sessions persist data between related calls.
  */
  kvds::kv_c* get_session_store();

  /*
    The runtime storage is to disk-back all runtime meta-configuration data for the server
  */
...
private:
  ...
  decltype(
    std::declval<kvds::kv_c_distributor_c>().
        get_or_create_kv_c(
          "", 
