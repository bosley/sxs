...
const char *system_c::get_name() const { return name_; }
  ...
  try {
    if (std::filesystem::exists(path)) {
      if (!std::filesystem::is_directory(path)) {
        logger_->error("[{}] Path exists but is not a directory: {}", name_,
                       path);
        return false;
      }
    ...
    logger_->error("[{}] Filesystem error for path {}: {}", name_, path,
                   e.what());
    return false;
  }
}
...
void system_c::initialize(runtime_accessor_t accessor) {
  ...
  logger_->info("[{}] Creating kvds distributor with base path: {}", name_,
                root_path_);
...
bool system_c::is_running() const { return running_; }

kvds::kv_c *system_c::get_entity_store() {
  return kv_entity_store_.has_value() ? kv_entity_store_.value().get()
                                      : nullptr;
}

kvds::kv_c *system_c::get_session_store() {
  return kv_session_store_.has_value() ? kv_session_store_.value().get()
                                       : nullptr;
}

kvds::kv_c *system_c::get_runtime_store() {
  return kv_runtime_store_.has_value() ? kv_runtime_store_.value().get()
                                       : nullptr;
}
