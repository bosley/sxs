...
std::string entity_c::get_schema() const {
  return R"([(permissions "[]") (topic_permissions "[]") (max_rps "0")])";
}

size_t entity_c::field_count() const { return 3; }
...
bool entity_c::load() {
  ...
  key = manager_->make_data_key(get_type_id(), instance_id_, 2);
  if (manager_->get_store().get(key, field_values_[2])) {
    try {
      max_rps_ = static_cast<std::uint32_t>(std::stoul(field_values_[2]));
    } catch (...) {
      max_rps_ = 0;
      field_values_[2] = "0";
    }
...
void entity_c::set_max_rps(std::uint32_t max_rps) {
  std::lock_guard<std::mutex> lock(publish_mutex_);
  max_rps_ = max_rps;
  field_values_[2] = std::to_string(max_rps);
}

std::uint32_t entity_c::get_max_rps() const {
  std::lock_guard<std::mutex> lock(publish_mutex_);
  return max_rps_;
}

void entity_c::cleanup_old_timestamps() {
  auto now = std::chrono::steady_clock::now();
  auto one_second_ago = now - std::chrono::seconds(1);

  while (!publish_timestamps_.empty() &&
         publish_timestamps_.front() < one_second_ago) {
    publish_timestamps_.pop_front();
  }
}

bool entity_c::try_publish() {
  std::lock_guard<std::mutex> lock(publish_mutex_);
  ...
  bool result = publish_timestamps_.size() < max_rps_;
  if (result) {
    publish_timestamps_.push_back(std::chrono::steady_clock::now());
  }
  return result;
}
