...
random_string_c::random_string_c()
    : _source_string(random_string_c::ALL_CHARS) {}
...
std::string random_string_c::generate_string(size_t length) const {
  ...
  for (auto i = 0; i < length; i++) {
    auto next_item = pkg::random::generate_random_c<size_t>().get_range(
        0, _source_string.size() - 1);
