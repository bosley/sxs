...
class record_manager_c {
public:
  explicit record_manager_c(kvds::kv_c &store,
  ...
  get_or_create(const std::string &instance_id) {
    static_assert(std::is_base_of<record_if, T>::value,
                  "T must inherit from record_if");
    ...
    if (exists(type_id, instance_id)) {
      if (!record->load()) {
        logger_->error("Failed to load existing record: {}:{}", type_id,
                       instance_id);
        return std::nullopt;
      }
    }
  ...
  void
  iterate_type(const std::string &type_id,
               std::function<bool(const std::string &instance_id)> callback);
  void iterate_all(std::function<bool(const std::string &type_id,
  ...
  std::string make_data_key(const std::string &type_id,
                            const std::string &instance_id,
                            size_t field_index) const;
  std::string make_data_prefix(const std::string &type_id) const;
  std::string make_instance_prefix(const std::string &type_id,
                                   const std::string &instance_id) const;
  std::string make_lock_key(const std::string &type_id,
                            const std::string &instance_id) const;

private:
  bool ensure_schema_registered(const std::string &type_id,
...
protected:
  ...
  void set_manager(record_manager_c *manager) override { manager_ = manager; }
