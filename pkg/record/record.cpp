...
record_manager_c::record_manager_c(kvds::kv_c &store,
...
std::string
record_manager_c::make_data_prefix(const std::string &type_id) const {
  return "record:data:" + type_id + ":";
}

std::string
record_manager_c::make_instance_prefix(const std::string &type_id,
                                       const std::string &instance_id) const {
  return "record:data:" + type_id + ":" + instance_id + ":";
}

std::string
record_manager_c::make_lock_key(const std::string &type_id,
                                const std::string &instance_id) const {
...
void record_manager_c::release_all_locks() {
  ...
  store_.iterate("record:lock:", [&lock_keys](const std::string &key,
  ...
  if (!lock_keys.empty()) {
    logger_->info("Released {} stale locks during initialization",
                  lock_keys.size());
  }
}

bool record_manager_c::ensure_schema_registered(const std::string &type_id,
  ...
  if (store_.exists(meta_key)) {
    ...
    if (existing_schema != schema) {
      logger_->error(
          "Schema mismatch for type: {}. Existing and new schemas differ",
          type_id);
      return false;
    }
...
bool record_manager_c::exists(const std::string &type_id,
                              const std::string &instance_id) {
...
bool record_manager_c::exists_any(const std::string &instance_id) {
  ...
  store_.iterate("record:meta:",
                 [&](const std::string &meta_key, const std::string &schema) {
...
void record_manager_c::iterate_type(
...
void record_manager_c::iterate_all(
    std::function<bool(const std::string &type_id,
                       const std::string &instance_id)>
        callback) {
  store_.iterate("record:meta:",
                 [&](const std::string &meta_key, const std::string &schema) {
...
bool record_base_c::del() {
  ...
  for (size_t i = 0; i < field_count(); ++i) {
    keys_to_delete.push_back(
        manager_->make_data_key(get_type_id(), instance_id_, i));
  }
