...
std::vector<std::string> get_context_lines(const std::string &source,
  ...
  size_t start_line = line_number > context_size ? line_number - context_size : 1;
...
std::string colorize_error_output(const formatted_error_s &error) {
  ...
  size_t context_start_line = error.error_line > error.context_lines.size() / 2
...
std::string format_type_error(const std::string &context_name,
  ...
  auto [line, col] = byte_offset_to_line_col(location.source_code, location.byte_offset);
  ...
  output << "\n\033[90mFile: " << location.file_path << ":" << line << ":" << col << "\033[0m\n";
