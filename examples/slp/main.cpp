...
public:
  ...
  std::variant<std::int64_t, double, std::string>
  eval(const slp::slp_object_c &obj) {
    auto type = obj.type();
  ...
  void execute(const slp::slp_object_c &obj) {
    ...
    if (type == slp::slp_type_e::BRACKET_LIST) {
      auto list = obj.as_list();
      std::cout << "Iterating over bracket list with " << list.size()
                << " elements" << std::endl;
...
int main(int argc, char **argv) {
  ...
  if (result.is_error()) {
    ...
    std::cerr << "At byte position: " << result.error().byte_position
              << std::endl;
    return 1;
  }
