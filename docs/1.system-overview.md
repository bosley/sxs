...
**kernel_manager_c** loads dynamic libraries and manages native function registration. Kernels are directories containing a kernel.sxs metadata file and a dylib. During initialization, the manager calls kernel_init in the dylib, passing a minimal C++ API table containing only register_function and eval. Kernels work directly with slp::slp_object_c objects using stack-based value semantics and static factory methods, eliminating all memory management concerns. Registered functions are stored in a registry under kernel_name/function_name. The interpreter can then call these native functions as if they were built-in language features. Kernels may optionally define a kernel_shutdown function which is automatically invoked before dylib unload if present, allowing kernels with global state to perform cleanup (the symbol is resolved via dlsym and silently skipped if not found).
...
When a kernel load is encountered, kernel_manager searches for the kernel directory, parses the metadata file, dlopen()s the dylib, and calls kernel_init. The kernel registers its functions through the C API, and these functions are wrapped in callable_symbol_s lambdas stored in the registry. During kernel_manager destruction, kernel_shutdown is called automatically for every kernel that defines it, before dylibs are closed.
