...
| Command | Syntax | Return | Description |
|---------|--------|--------|-------------|
| `eq` | `(eq lhs rhs)` | `:int` | Compare two values for equality. Returns 1 if equal, 0 if not. Accepts `:any` type for both arguments. Type must match for equality. Lists are compared deeply (recursive element comparison). Lambdas match by identity (same lambda ID). Error objects match by message string. |
...
(def same (eq 42 42))
(def diff (eq 42 43))
(def type-diff (eq 42 "42"))
(def list-eq (eq {1 2 3} {1 2 3}))

(def add (fn (x :int) :int [x]))
(def same-fn (eq add add))
```
