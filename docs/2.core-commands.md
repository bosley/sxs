...
| `match` | `(match value (pattern result)...)` | `:aberrant` | Pattern matching on values. Returns result of first matching pattern or `@(no matching handler found)`. |
| `reflect` | `(reflect value (:type body)...)` | `:aberrant` | Type-based dispatch. Evaluates body for matching type or returns `@(handler not supplied for given type)`. |
