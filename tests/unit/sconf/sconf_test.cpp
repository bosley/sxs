...
TEST_CASE("sconf non-homogeneous list error", "[unit][sconf][error]") {
    ...
    CHECK(result.is_error());
    CHECK(result.error().error_code == sconf::sconf_error_e::INVALID_LIST_ELEMENT);
...
TEST_CASE("sconf invalid structure triple instead of pair", "[unit][sconf][error]") {
...
TEST_CASE("sconf all integer types", "[unit][sconf][types]") {
    std::string source = "[(i8 1) (i16 2) (i32 3) (i64 4) (u8 5) (u16 6) (u32 7) (u64 8)]";
...
TEST_CASE("sconf builder chaining", "[unit][sconf][builder]") {
    std::string source = R"([(host "localhost") (port 8080) (workers (1 2 3 4))])";
...
TEST_CASE("sconf list of lists with non-list element error", "[unit][sconf][error]") {
