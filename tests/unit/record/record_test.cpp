...
public:
  user_record_c() { field_values_.resize(3); }

  std::string get_type_id() const override { return "user"; }
...
public:
  product_record_c() { field_values_.resize(2); }

  std::string get_type_id() const override { return "product"; }
...
TEST_CASE("record type iteration", "[unit][record]") {
  ...
  SECTION("iterate instances of a type") {
    ...
    CHECK(std::find(instance_ids.begin(), instance_ids.end(), "jack") !=
          instance_ids.end());
    CHECK(std::find(instance_ids.begin(), instance_ids.end(), "jill") !=
          instance_ids.end());
    CHECK(std::find(instance_ids.begin(), instance_ids.end(), "john") !=
          instance_ids.end());
  }
  ...
  SECTION("early termination in iteration") {
    for (int i = 0; i < 5; ++i) {
      auto user =
          manager.get_or_create<user_record_c>("user_" + std::to_string(i));
...
TEST_CASE("record iteration across all types", "[unit][record]") {
  ...
  SECTION("iterate all records") {
    ...
    manager.iterate_all(
        [&records](const std::string &type_id, const std::string &instance_id) {
...
TEST_CASE("record locking acquisition and release", "[unit][record][locking]") {
  ...
  std::string test_db_path =
      get_unique_test_path("/tmp/record_test_lock_acquire");
  auto logger = create_test_logger();
...
TEST_CASE("record locking prevents concurrent modification",
          "[unit][record][locking]") {
  kvds::datastore_c ds;
  std::string test_db_path =
      get_unique_test_path("/tmp/record_test_lock_contention");
  auto logger = create_test_logger();
...
TEST_CASE("record locking cleanup on delete", "[unit][record][locking]") {
  kvds::datastore_c ds;
  std::string test_db_path =
      get_unique_test_path("/tmp/record_test_lock_cleanup");
  auto logger = create_test_logger();
...
TEST_CASE("record locking with multiple record types",
          "[unit][record][locking]") {
  kvds::datastore_c ds;
  std::string test_db_path =
      get_unique_test_path("/tmp/record_test_lock_multi");
  auto logger = create_test_logger();
...
TEST_CASE("record manager releases all locks on startup",
          "[unit][record][locking]") {
  kvds::datastore_c ds;
  std::string test_db_path =
      get_unique_test_path("/tmp/record_test_lock_startup");
  auto logger = create_test_logger();
