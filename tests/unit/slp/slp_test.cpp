...
public:
    static const std::vector<std::uint8_t>& get_data(const slp::slp_object_c& obj) {
        return obj.data_;
    }
    
    static const std::map<std::uint64_t, std::string>& get_symbols(const slp::slp_object_c& obj) {
        return obj.symbols_;
    }
    
    static const slp::slp_unit_of_store_t* get_view(const slp::slp_object_c& obj) {
        return obj.view_;
    }
...
TEST_CASE("slp list operations - invalid type", "[unit][slp][list][interface]") {
...
TEST_CASE("slp string operations - invalid type", "[unit][slp][string][interface]") {
...
TEST_CASE("slp internal structure validation", "[unit][slp][internal]") {
    SECTION("integer internal structure") {
        ...
        CHECK(view != nullptr);
        CHECK((view->header & 0xFF) == static_cast<std::uint32_t>(slp::slp_type_e::INTEGER));
    }
