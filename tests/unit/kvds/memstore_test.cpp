...
TEST_CASE("memstore iteration", "[unit][kvds][memstore]") {
  ...
  SECTION("prefix iteration") {
    ...
    ms.iterate("user:", [&found_users](const std::string &key,
    ...
    ms.iterate("group:", [&found_groups](const std::string &key,
                                         const std::string &value) {
  ...
  SECTION("early termination") {
    ...
    int count = 0;
    ms.iterate("test",
               [&count](const std::string &key, const std::string &value) {
                 count++;
                 return count < 2;
               });
...
TEST_CASE("memstore operations on closed store", "[unit][kvds][memstore]") {
  ...
  SECTION("all operations fail when closed") {
    ...
    ms.iterate("", [&callback_called](const std::string &key,
                                      const std::string &value) {
