...
TEST_CASE("explicit key scoping verification", "[unit][runtime][session]") {
  ...
  std::string entity_test_path =
      get_unique_test_path("/tmp/session_test_scoping_entity");
  std::string data_test_path =
      get_unique_test_path("/tmp/session_test_scoping_data");
  auto logger = create_test_logger();
...
TEST_CASE("key masking on iteration", "[unit][runtime][session]") {
  ...
  kvds::datastore_c data_ds;
  std::string entity_test_path =
      get_unique_test_path("/tmp/session_test_masking_entity");
  std::string data_test_path =
      get_unique_test_path("/tmp/session_test_masking_data");
  auto logger = create_test_logger();
  ...
  SECTION("iteration returns unscoped keys") {
    ...
    store->iterate("",
                   [&keys](const std::string &key, const std::string &value) {
                     keys.push_back(key);
                     return true;
                   });
  ...
  SECTION("iteration with prefix filters within session scope") {
    ...
    store->iterate("user:", [&user_keys](const std::string &key,
    ...
    CHECK(std::find(user_keys.begin(), user_keys.end(), "user:alice") !=
          user_keys.end());
    CHECK(std::find(user_keys.begin(), user_keys.end(), "user:bob") !=
          user_keys.end());
    CHECK(std::find(user_keys.begin(), user_keys.end(), "config:setting") ==
          user_keys.end());
  }
  ...
  SECTION("iteration only sees keys from session scope") {
    ...
    session1.get_store()->iterate(
        "", [&keys_a](const std::string &key, const std::string &value) {
          keys_a.push_back(key);
          return true;
        });

    std::vector<std::string> keys_b;
    session2.get_store()->iterate(
        "", [&keys_b](const std::string &key, const std::string &value) {
...
TEST_CASE("session time tracking", "[unit][runtime][session]") {
  ...
  kvds::datastore_c data_ds;
  std::string entity_test_path =
      get_unique_test_path("/tmp/session_test_time_entity");
  std::string data_test_path =
      get_unique_test_path("/tmp/session_test_time_data");
  auto logger = create_test_logger();
...
TEST_CASE("comprehensive kv operations", "[unit][runtime][session]") {
  ...
  kvds::datastore_c data_ds;
  std::string entity_test_path =
      get_unique_test_path("/tmp/session_test_ops_entity");
  std::string data_test_path =
      get_unique_test_path("/tmp/session_test_ops_data");
  auto logger = create_test_logger();
...
TEST_CASE("permission boundary tests", "[unit][runtime][session]") {
  ...
  kvds::datastore_c data_ds;
  std::string entity_test_path =
      get_unique_test_path("/tmp/session_test_perm_entity");
  std::string data_test_path =
      get_unique_test_path("/tmp/session_test_perm_data");
  auto logger = create_test_logger();
  ...
  SECTION("read-only permission blocks writes") {
    auto entity_opt =
        entity_manager.get_or_create<runtime::entity_c>("readonly_user");
    REQUIRE(entity_opt.has_value());
    auto entity = std::move(entity_opt.value());
  ...
  SECTION("write-only permission blocks reads") {
    auto entity_opt =
        entity_manager.get_or_create<runtime::entity_c>("writeonly_user");
    REQUIRE(entity_opt.has_value());
    auto entity = std::move(entity_opt.value());

    entity->grant_permission("writeonly_scope",
                             runtime::permission::WRITE_ONLY);
    entity->save();
  ...
  SECTION("read-write permission allows all operations") {
    auto entity_opt =
        entity_manager.get_or_create<runtime::entity_c>("rw_user");
    REQUIRE(entity_opt.has_value());
    auto entity = std::move(entity_opt.value());
    ...
    std::map<std::string, std::string> batch = {{"key2", "val2"},
                                                {"key3", "val3"}};
    CHECK(store->set_batch(batch));
...
TEST_CASE("session edge cases", "[unit][runtime][session]") {
  ...
  kvds::datastore_c data_ds;
  std::string entity_test_path =
      get_unique_test_path("/tmp/session_test_edge_entity");
  std::string data_test_path =
      get_unique_test_path("/tmp/session_test_edge_data");
  auto logger = create_test_logger();
...
TEST_CASE("session subsystem management", "[unit][runtime][session]") {
  ...
  kvds::datastore_c data_ds;
  std::string entity_test_path =
      get_unique_test_path("/tmp/session_test_mgmt_entity");
  std::string session_test_path =
      get_unique_test_path("/tmp/session_test_mgmt_session");
  std::string data_test_path =
      get_unique_test_path("/tmp/session_test_mgmt_data");
  auto logger = create_test_logger();
...
TEST_CASE("session lifecycle management", "[unit][runtime][session]") {
  ...
  kvds::datastore_c data_ds;
  std::string entity_test_path =
      get_unique_test_path("/tmp/session_test_lifecycle_entity");
  std::string data_test_path =
      get_unique_test_path("/tmp/session_test_lifecycle_data");
  auto logger = create_test_logger();
  ...
  SECTION("session data persists in scoped datastore") {
    ...
    REQUIRE(entity_opt.has_value());
    auto entity = std::move(entity_opt.value());
    entity->grant_permission("persistent_scope",
