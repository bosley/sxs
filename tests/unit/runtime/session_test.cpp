...
TEST_CASE("explicit key scoping verification", "[unit][runtime][session]") {
  ...
  SECTION("keys are prefixed with scope in underlying store") {
    ...
    runtime::session_c session("sess1", "user1", "my_scope", entity.get(), &data_ds);
  ...
  SECTION("different scopes don't interfere") {
    ...
    runtime::session_c session1("sess1", "user1", "scope_a", entity1.get(), &data_ds);
    runtime::session_c session2("sess2", "user2", "scope_b", entity2.get(), &data_ds);
  ...
  SECTION("multiple sessions same scope share data") {
    ...
    runtime::session_c session1("sess1", "user1", "shared_scope", entity.get(), &data_ds);
    runtime::session_c session2("sess2", "user1", "shared_scope", entity.get(), &data_ds);
...
TEST_CASE("key masking on iteration", "[unit][runtime][session]") {
  ...
  SECTION("iteration returns unscoped keys") {
    ...
    runtime::session_c session("sess1", "user1", "test_scope", entity.get(), &data_ds);
    auto* store = session.get_store();
...
TEST_CASE("session time tracking", "[unit][runtime][session]") {
  ...
  SECTION("multiple sessions have different creation times") {
    runtime::session_c session1("sess1", "user1", "test_scope", entity.get(), &data_ds);
    auto time1 = session1.get_creation_time();
    
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    
    runtime::session_c session2("sess2", "user1", "test_scope", entity.get(), &data_ds);
...
TEST_CASE("session edge cases", "[unit][runtime][session]") {
  ...
  SECTION("session metadata accessors") {
    runtime::session_c session("sess123", "user456", "scope789", entity.get(), &data_ds);
...
TEST_CASE("session lifecycle management", "[unit][runtime][session]") {
  ...
  SECTION("multiple sessions can be tracked") {
    ...
    runtime::session_c session1("sess1", "user1", "scope1", entity1.get(), &data_ds);
    runtime::session_c session2("sess2", "user2", "scope2", entity2.get(), &data_ds);
    runtime::session_c session3("sess3", "user1", "scope1", entity1.get(), &data_ds);
  ...
  SECTION("session data persists in scoped datastore") {
    ...
    {
      runtime::session_c session("sess1", "user1", "persistent_scope", entity.get(), &data_ds);
    ...
    {
      runtime::session_c new_session("sess2", "user1", "persistent_scope", entity.get(), &data_ds);
