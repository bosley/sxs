...
TEST_CASE("entity permission checking", "[unit][runtime][entity]") {
  ...
  SECTION("read-only permission does not grant write") {
    ...
    CHECK_FALSE(
        (*entity)->is_permitted("data", runtime::permission::WRITE_ONLY));
  }
  ...
  SECTION("write-only permission does not grant read") {
    ...
    CHECK_FALSE(
        (*entity)->is_permitted("data", runtime::permission::READ_ONLY));
  }
  ...
  SECTION("no permission by default") {
    ...
    CHECK_FALSE(
        (*entity)->is_permitted("nonexistent", runtime::permission::READ_ONLY));
  }
...
TEST_CASE("entity permission revocation", "[unit][runtime][entity]") {
  ...
  SECTION("revoke granted permission") {
    ...
    CHECK_FALSE(
        (*entity)->is_permitted("scope1", runtime::permission::READ_ONLY));
  }
  ...
  SECTION("revoke one permission leaves others intact") {
    ...
    CHECK_FALSE(
        (*entity)->is_permitted("scope_a", runtime::permission::READ_ONLY));
    CHECK((*entity)->is_permitted("scope_b", runtime::permission::WRITE_ONLY));
  }
...
TEST_CASE("entity get and set permissions", "[unit][runtime][entity]") {
  ...
  SECTION("set permissions replaces all permissions") {
    ...
    CHECK_FALSE(
        (*entity)->is_permitted("old_scope", runtime::permission::READ_ONLY));
    CHECK(
        (*entity)->is_permitted("new_scope1", runtime::permission::READ_ONLY));
    CHECK(
        (*entity)->is_permitted("new_scope2", runtime::permission::READ_WRITE));
  }
...
TEST_CASE("multiple entities with different permissions",
          "[unit][runtime][entity]") {
  kvds::datastore_c ds;
  ...
  SECTION("different users have independent permissions") {
    ...
    CHECK_FALSE(
        (*user1)->is_permitted("project_b", runtime::permission::READ_ONLY));

    CHECK((*user2)->is_permitted("project_b", runtime::permission::READ_ONLY));
    CHECK_FALSE(
        (*user2)->is_permitted("project_a", runtime::permission::READ_WRITE));
  }
