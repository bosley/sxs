...
TEST_CASE("entity topic permission granting", "[unit][runtime][session][events]") {
  kvds::datastore_c entity_ds;
  std::string entity_test_path = get_unique_test_path("/tmp/session_event_test_grant_entity");
  ...
  SECTION("can grant subscribe permission") {
    ...
    CHECK(entity->is_permitted_topic(200, runtime::topic_permission::SUBSCRIBE));
  }
  ...
  SECTION("can grant pubsub permission") {
    ...
    CHECK(entity->is_permitted_topic(300, runtime::topic_permission::SUBSCRIBE));
  }
  ...
  SECTION("multiple topic permissions are independent") {
    ...
    CHECK(entity->is_permitted_topic(100, runtime::topic_permission::PUBLISH));
    CHECK_FALSE(entity->is_permitted_topic(100, runtime::topic_permission::SUBSCRIBE));
    
    CHECK_FALSE(entity->is_permitted_topic(200, runtime::topic_permission::PUBLISH));
    CHECK(entity->is_permitted_topic(200, runtime::topic_permission::SUBSCRIBE));
...
TEST_CASE("entity topic permission revocation", "[unit][runtime][session][events]") {
  kvds::datastore_c entity_ds;
  std::string entity_test_path = get_unique_test_path("/tmp/session_event_test_revoke_entity");
  auto logger = create_test_logger();
  ...
  SECTION("can revoke granted permission") {
    ...
    CHECK_FALSE(entity->is_permitted_topic(100, runtime::topic_permission::PUBLISH));
  }
...
TEST_CASE("entity topic permission persistence", "[unit][runtime][session][events]") {
  kvds::datastore_c entity_ds;
  std::string entity_test_path = get_unique_test_path("/tmp/session_event_test_persist_entity");
  auto logger = create_test_logger();
...
TEST_CASE("session publish with permissions", "[unit][runtime][session][events]") {
  kvds::datastore_c entity_ds;
  kvds::datastore_c data_ds;
  std::string entity_test_path = get_unique_test_path("/tmp/session_event_test_pub_entity");
  std::string data_test_path = get_unique_test_path("/tmp/session_event_test_pub_data");
  auto logger = create_test_logger();
...
TEST_CASE("session subscribe with permissions", "[unit][runtime][session][events]") {
  kvds::datastore_c entity_ds;
  kvds::datastore_c data_ds;
  std::string entity_test_path = get_unique_test_path("/tmp/session_event_test_sub_entity");
  std::string data_test_path = get_unique_test_path("/tmp/session_event_test_sub_data");
  auto logger = create_test_logger();
  ...
  SECTION("can subscribe with subscribe permission") {
    ...
    CHECK(session.subscribe_to_topic(200, handler));
  }
  ...
  SECTION("can subscribe with pubsub permission") {
    ...
    CHECK(session.subscribe_to_topic(201, handler));
  }
  ...
  SECTION("cannot subscribe without permission") {
    ...
    CHECK_FALSE(session.subscribe_to_topic(202, handler));
  }
  ...
  SECTION("cannot subscribe with only publish permission") {
    ...
    CHECK_FALSE(session.subscribe_to_topic(203, handler));
  }
...
TEST_CASE("session event publish and consume", "[unit][runtime][session][events]") {
  kvds::datastore_c entity_ds;
  kvds::datastore_c data_ds;
  std::string entity_test_path = get_unique_test_path("/tmp/session_event_test_pubsub_entity");
  std::string data_test_path = get_unique_test_path("/tmp/session_event_test_pubsub_data");
  auto logger = create_test_logger();
  ...
  SECTION("single session can publish and receive its own events") {
    ...
    auto handler = [&event_count, &received_payload](const runtime::events::event_s& event) {
      event_count++;
  ...
  SECTION("multiple sessions can communicate via events") {
    ...
    auto handler2 = [&session2_event_count, &session2_payload](const runtime::events::event_s& event) {
  ...
  SECTION("multiple subscribers receive same event") {
    ...
    auto handler2 = [&count2](const runtime::events::event_s& event) { count2++; };
    auto handler3 = [&count3](const runtime::events::event_s& event) { count3++; };
...
TEST_CASE("session unsubscribe from topics", "[unit][runtime][session][events]") {
  kvds::datastore_c entity_ds;
  kvds::datastore_c data_ds;
  std::string entity_test_path = get_unique_test_path("/tmp/session_event_test_unsub_entity");
  std::string data_test_path = get_unique_test_path("/tmp/session_event_test_unsub_data");
  auto logger = create_test_logger();
...
TEST_CASE("session multiple topic subscriptions", "[unit][runtime][session][events]") {
  kvds::datastore_c entity_ds;
  kvds::datastore_c data_ds;
  std::string entity_test_path = get_unique_test_path("/tmp/session_event_test_multi_entity");
  std::string data_test_path = get_unique_test_path("/tmp/session_event_test_multi_data");
  auto logger = create_test_logger();
  ...
  CHECK(session.subscribe_to_topic(700, [&count700](const runtime::events::event_s& e) { count700++; }));
...
TEST_CASE("session event payload types", "[unit][runtime][session][events]") {
  kvds::datastore_c entity_ds;
  kvds::datastore_c data_ds;
  std::string entity_test_path = get_unique_test_path("/tmp/session_event_test_payload_entity");
  std::string data_test_path = get_unique_test_path("/tmp/session_event_test_payload_data");
  auto logger = create_test_logger();
